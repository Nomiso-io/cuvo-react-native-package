...
#define DEFAULT_AUTOSAVE_DURATION 1800
...
@implementation CuvoScreenRecorder {
...
- (instancetype)initWithWindow:(UIWindow *)window
{
    ...
    if (self) {
        ...
        counter++;
        NSString *label = [NSString stringWithFormat:@"com.cuvo.screen_recorder-%@", @(counter)];
...
- (void)setupTouchPointer
{
    if (self.showsTouchPointer) {
        CuvoTouchPointerWindowInstall();
    } else {
        CuvoTouchPointerWindowUninstall();
    }
}
...
- (void)stopRecording :(void (^)(NSString *output))completionHandler
{
    ...
    startTimestamp = 0.0;
    NSLog(@"stopRecordingstopRecordingstopRecording ----------------- ");
...
- (NSURL *)outputFileURL
{
    if (!self.filenameBlock) {
        __block CuvoScreenRecorder *wself = self;
