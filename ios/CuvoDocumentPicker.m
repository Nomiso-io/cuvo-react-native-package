...
#import "CuvoDocumentPicker.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import <React/RCTConvert.h>
#import <React/RCTBridge.h>
#import <React/RCTUtils.h>
#import "CuvoPromiseWrapper.h"
...
@interface CuvoDocumentPicker () <UIDocumentPickerDelegate, UIAdaptivePresentationControllerDelegate>
@end

@implementation CuvoDocumentPicker {
    UIDocumentPickerMode mode;
    NSString *copyDestination;
    CuvoPromiseWrapper* promiseWrapper;
...
{
    if ((self = [super init])) {
        promiseWrapper = [CuvoPromiseWrapper new];
...
- (NSMutableDictionary *)getMetadataForUrl:(NSURL *)url error:(NSError **)error
{
    ...
    [coordinator coordinateReadingItemAtURL:url options:NSFileCoordinatorReadingResolvesSymbolicLink error:&fileError byAccessor:^(NSURL *newURL) {
        ...
        NSError *copyError;
        NSString *maybeFileCopyPath = copyDestination ? [CuvoDocumentPicker copyToUniqueDestinationFrom:newURL usingDestinationPreset:copyDestination error:copyError].absoluteString : nil;
        ...
        if(!attributesError) {
            ...
            NSLog(@"CuvoDocumentPicker: %@", attributesError);
        }
